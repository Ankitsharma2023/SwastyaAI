...
>
	<Sidebar asideClass="w-52">
		...
		>
			<SidebarGroup>
				<div class="flex w-full flex-row items-center justify-start">
					<SidebarItem
						class="-mb-2 ml-0 h-8 w-40 font-medium text-white hover:text-black"
						label="Home"
						{spanClass}
						on:click={() => {
							toggleSidebar();
				...
				<SidebarItem
					class="mt-4 ml-0 h-8 w-40 font-medium text-white hover:text-black"
					label="Chatbot"
					{spanClass}
					on:click={() => {
						toggleSidebar();
				...
				<SidebarItem
					class="ml-0 h-8 w-40 font-medium text-white hover:text-black"
					label="Recording"
					{spanClass}
					on:click={() => {
						toggleSidebar();
					...
					<svelte:fragment slot="icon">
						...
						>
							...
							<path
								...
								stroke-linejoin="round"
							/>
							<path d="M7.62575 15.3345L4.63281 17.7573" stroke="blue" stroke-linejoin="round" />
						</svg>
					</svelte:fragment>
				</SidebarItem>
				<SidebarItem
		...
		<div class="-mt-3 mb-2 ml-2 w-48 border-[1px] border-white"></div>
		<p class="text-md mb-1 ml-4 font-semibold text-white">History</p>

		<SidebarWrapper
			divClass="overflow-y-auto py-4 px-3 rounded dark:bg-gray-800 flex flex-col gap-1 w-full h-[25vh]"
		>
			{#each Object.entries(previousChats) as [chat, val] (val)}
				<SidebarGroup>
					<div
...
<div
	class="dark:bg-dark flex items-center justify-between p-4 text-black dark:text-white"
	style="background-color: #65A765;"
>
	<div class="flex items-center justify-evenly gap-2">
		...
		<h1 class="ml-6 text-2xl font-bold text-white">SwasthAI</h1>
	</div>
