...
>
	<Sidebar asideClass="w-52">
		...
		>
			<SidebarGroup>
				<div class="flex flex-row justify-start items-center w-full">
					<SidebarItem
						class="ml-0 font-medium w-40 h-8 -mb-2 text-[#004E86]"
						label="Home"
						{spanClass}
						on:click={() => {
							toggleSidebar();
				...
				<SidebarItem
					class="ml-0 font-medium mt-4 w-40 h-8 text-[#004E86]"
					label="Chatbot"
					{spanClass}
					on:click={() => {
						toggleSidebar();
				...
				<SidebarItem
					class="ml-0 font-medium w-40 h-8 text-[#004E86]"
					label="Recording"
					{spanClass}
					on:click={() => {
						toggleSidebar();
		...
		<div class="w-48 ml-2 border-[1px] border-[#93B6CF] mb-2 -mt-3"></div>
			...
			{#each Object.entries(previousChats) as [chat, val]}
				<SidebarGroup>
					...
					>
						...
						><button class="text-black z-20 -mr-4" on:click={() => removeChat(chat)}
							><svg
								xmlns="http://www.w3.org/2000/svg"
								fill="none"
								viewBox="0 0 24 24"
...
<div class="p-4 dark:bg-dark bg-white dark:text-white text-black flex justify-between items-center">
	<div class="gap-2 flex justify-evenly items-center">
		...
		<h1 class="font-bold ml-6 text-2xl">SwasthAI</h1>
	</div>
				...
				{#each notifications as n}
					<DropdownItem class="flex space-x-4 rtl:space-x-reverse">
						<div class="ps-3 w-full">
							<div class="text-black text-sm mb-1.5 dark:text-gray-400">
								{n}
							</div>
						</div>
					</DropdownItem>
		...
		{#if $userStore}
			<div class="flex flex-row justify-center items-center">
				...
				>
					...
					>
					<DropdownItem class="flex flex-row gap-2 items-center px-2">
						<DarkMode />
					...
					<DropdownDivider />
					<DropdownItem on:click={signOut} slots="footer" class="flex flex-row gap-2 items-center">
						<svg
							width="21"
							height="20"
							viewBox="0 0 21 20"
